...
基于 Kotlin + Jetpack Compose 开发的健康管理应用，对标 iOS SwiftUI 客户端。提供个人健康档案管理、家庭成员关怀、AI 数字人健康咨询、用药提醒、就医预约等一站式健康服务。

*Kotlin + Jetpack Compose health management app, mirroring the iOS SwiftUI counterpart. One-stop health services including personal health records, family care, AI health consultation, medication reminders, and appointment scheduling.*
...
### 个人中心 | Profile
- 头像上传与裁剪（自定义裁剪器，双指缩放 + 拖拽，圆形裁剪，对标 iOS ImageCropperView）
- 基本信息编辑（用户名、个性签名、性别、出生日期）
- 健康档案（身高、体重、血型、疾病史、过敏史、手术史、用药史、慢性病、备注）
- 个人资料完善度提示

*Avatar upload & crop (custom cropper with pinch-zoom, drag, circular crop matching iOS); basic info editing; health profile (height, weight, blood type, medical/allergy/surgery/medication/chronic history, notes); profile completion tips.*
...
| --- | --- |
| 语言 Language | Kotlin |
| UI 框架 UI Framework | Jetpack Compose（对标 SwiftUI） |
...
| CachedAsyncImage | Coil AsyncImage |
| ImageCropperView | ImageCropperScreen |
...
> iOS 端已上架 App Store，Android 端正按 iOS 功能对标推进。
> *The iOS version is published on the App Store. Android development follows iOS feature parity.*
